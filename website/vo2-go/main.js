// ----------------------------------------------------------------
// Load WASM (TinyGo build)
//
// TinyGo's wasm_exec.js shim defines globalThis.Go and provides the
// import object the runtime needs (syscall/js, memory, etc). The Go
...
(async () => {
  try {
    if (typeof globalThis.Go !== 'function') {
      throw new Error('wasm_exec.js did not load — make sure the script tag is present in index.html');
    }
    ...
    go.run(result.instance); // intentionally not awaited
    analyze_gpx = globalThis.analyze_gpx;
