// ----------------------------------------------------------------
// Load analyser (pure JS — no WASM)
// ----------------------------------------------------------------

// No WASM in this build. The constant keeps the same name as the other
// builds for parity; the value is the JS analyser bundle size, which is
// the comparable "bytes shipped to run analysis" measurement.
const WASM_SIZE_BYTES = 24_562;
...
(async () => {
  try {
    const mod = await import('./vo2/index.js');
    analyze_gpx = mod.analyzeGpx;
    wasmReady = true;
    updateBtn();
  } catch (e) {
    console.error('Analyser load failed:', e);
    showStatus('error', 'Failed to load analyser module. See console for details.');
  }
})();
...
analyzeBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // Pure-JS analyser returns the result object directly — no JSON round-trip.
    const data = analyze_gpx(text, weightKg, maxHr);
