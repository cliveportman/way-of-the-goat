...
// Load WASM (Zig build)
//
// Zig produces a bare WebAssembly module with no auto-generated JS shim,
// so we hand-marshal strings across linear memory:
//   alloc(len)  -> ptr           — allocate a buffer in WASM memory
//   free(ptr, len)               — release a buffer
//   analyze_gpx(ptr, len, w, hr) -> out_len
//   get_result_ptr() -> out_ptr  — pointer to the JSON output
// ----------------------------------------------------------------

// Size of pkg/vo2.wasm in bytes. Update via build.sh after each Zig build.
const WASM_SIZE_BYTES = 88_262;
...
(async () => {
  try {
    ...
    const { memory, alloc, free, analyze_gpx: ag, get_result_ptr } = instance.exports;

    analyze_gpx = (gpx, weight, maxHr) => {
      const bytes = enc.encode(gpx);
      const ptr = alloc(bytes.length);
      if (ptr === 0) throw new Error('WASM alloc failed');
      new Uint8Array(memory.buffer, ptr, bytes.length).set(bytes);
      const outLen = ag(ptr, bytes.length, weight, maxHr | 0);
      free(ptr, bytes.length);
      if (outLen === 0) return '{"error":"analyze_gpx returned no result"}';
      const outPtr = get_result_ptr();
      // .slice() detaches from the WASM memory buffer so the caller can
      // hold the bytes safely across subsequent allocations.
