// ----------------------------------------------------------------
// Load WASM (.NET wasmbrowser build)
//
// The .NET runtime bootstraps via _framework/dotnet.js. Once exports are
// resolved, Vo2Api.AnalyzeGpx is a synchronous string-returning function
// we can call from the same UI flow as the Rust/Go builds.
// ----------------------------------------------------------------
// Total bytes of every .wasm in dist/wwwroot/_framework/ — the .NET runtime
// plus every trimmed assembly. Update via build.sh after publishing.
const WASM_SIZE_BYTES = 8_032_500;
...
(async () => {
  try {
    ...
    analyze_gpx = (gpx, weight, maxHr) =>
      exports.Vo2Api.AnalyzeGpx(gpx, weight, maxHr | 0);
    wasmReady = true;
    updateBtn();
  } catch (e) {
