// ----------------------------------------------------------------
// Load WASM (Kotlin/Wasm build)
//
// Webpack emits a UMD bundle that registers itself on globalThis["vo2-kt"].
// The value is a Promise (the entry module loads the wasm asynchronously),
// so we await it to get the actual analyze_gpx export.
// ----------------------------------------------------------------

// Size of dist/vo2.wasm — update via build.sh after each Gradle build.
const WASM_SIZE_BYTES = 130_663;
...
(async () => {
  try {
    if (!globalThis['vo2-kt']) {
      throw new Error('vo2.js did not load — make sure the script tag is present in index.html');
    }
    const mod = await globalThis['vo2-kt'];
    analyze_gpx = mod.analyze_gpx;
    if (typeof analyze_gpx !== 'function') {
      throw new Error('analyze_gpx was not registered by the WASM module');
    }
    wasmReady = true;
    updateBtn();
  } catch (e) {
